...
const PublishForm = ({ onCancel, onConfirm, show }) => {
    ...
    const [apiErrors, setApiErrors] = useState([]); // State untuk menampung missing_data dari API
    ...
    useEffect(() => {
        const fetchMissingData = async () => {
            ...
            setIsLoading(true);
            try {
                // Endpoint: /events/{id}/publish-status
                const response = await api.get(
                    `/events/${eventId}/publish-status`,
                );

                if (response.data.success) {
                    // Jika is_ready_to_publish false, ambil missing_data
                    setApiErrors(response.data.data.missing_data || []);
                }
            } catch (error) {
    ...
    // 2. Reset state saat modal ditutup/dibuka
    useEffect(() => {
        if (show) {
            setSlug("");
            setTouched(false);
            setApiErrors([]); // Bersihkan error lama
        }
    }, [show]);
    ...
    const allErrors = [
        ...apiErrors,
        ...(showSlugErr ? ["Slug / URL event belum diisi"] : []),
    ];
    ...
    return (
        <>
            {/* ① Warning banner */}
            <Alert
                className="d-flex gap-2 align-items-start mb-3 border"
                style={{
            ...
            <Form.Group>
                ...
                {showSlugErr ? (
                    ...
                    >
                        <CheckCircle2 size={12} color="#0089cb" /> Slug ini akan
                        menjadi URL permanen untuk event Anda.
                    </div>
                )}
            </Form.Group>
