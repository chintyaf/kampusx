...
const MapVisualizer = ({ locationData, setLocationData, isManualMode, isLocationSelected }) => {
	...
	const MapEvents = () => {
		const map = useMap();
		useEffect(() => {
			if (isLocationSelected) map.flyTo([locationData.latitude, locationData.longitude], 16);
			...
			{locationData && (
				<div
					...
					style={{
						height: '400px',
						zIndex: 0,
						border: `1px solid ${isManualMode ? 'var(--color-primary)' : 'var(--color-border)'}`,
					}}
				>
					<MapContainer
						center={
