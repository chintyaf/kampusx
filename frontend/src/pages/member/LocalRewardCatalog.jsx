...
import { Gift, Coins, Image as ImageIcon, AlertCircle, ShoppingBag, ArrowLeft } from 'lucide-react';
...
const LocalRewardCatalog = () => {
	...
	const loadData = async () => {
		try {
			setIsLoading(true);
			setError(null);
			await fetchLocalRewardsAndBalance();
		} catch (err) {
	...
	const fetchLocalRewardsAndBalance = async () => {
		try {
			const response = await api.get(`/events/${id}/local-rewards`);
			if (response.data?.success) {
				setRewards(response.data.data.rewards || []);
				setBalance(response.data.data.local_balance || 0);
			}
		} catch (err) {
			console.error('Gagal memuat rewards dan saldo poin:', err);
			throw err;
		}
	};
	...
	const handleRedeem = async () => {
		...
		try {
			setSubmitting(true);
			const response = await api.post(`/events/${id}/local-rewards/redeem`, {
				reward_id: selectedReward.id,
				notes: notes,
			});

			if (response.data?.success) {
				notify('success', 'Sukses', 'Reward lokal berhasil ditukarkan!');
				setShowConfirmModal(false);
				// Refresh saldo & katalog setelah sukses
				fetchLocalRewardsAndBalance();
			}
		} catch (err) {
	...
	return (
		<Container className="py-4 py-md-5">
			...
			<Button
				...
				onClick={() => navigate(`/event-space/${id}`)}
				style={{ borderRadius: '10px', fontSize: '13px' }}
			>
				<ArrowLeft size={16} /> Kembali ke Event Space
			</Button>
			...
			<div className="d-flex justify-content-between align-items-center mb-4">
				<div>
					<h3 className="fw-bold text-dark mb-1">Katalog Local Reward</h3>
					<p className="text-muted mb-0 small">Tukarkan poin lokal event yang telah Anda kumpulkan dengan berbagai merchandise menarik khusus event ini</p>
				</div>
			</div>
			...
			{isLoading ? (
				...
				<div className="text-center py-5 text-muted border rounded-3 bg-light d-flex flex-column align-items-center justify-content-center">
					...
					<h6 className="fw-bold">Belum Ada Reward Lokal Tersedia</h6>
					<p className="small mb-0">Silakan kembali lagi nanti untuk melihat katalog hadiah terbaru khusus event ini.</p>
				</div>
			) : (
				<Row className="g-4">
					{rewards.map((reward) => {
								...
								<Card className="h-100 shadow-sm border-0 transition-all hover-lift" style={{ borderRadius: '14px', overflow: 'hidden' }}>
									...
									<Card.Body className="d-flex flex-column p-3">
										...
										<div className="border-top pt-3 mt-auto">
											...
											<Button 
												variant={isOutOfStock || isInsufficientBalance ? 'secondary' : 'teal'}
												className="w-100 fw-bold py-2 shadow-sm d-flex align-items-center justify-content-center gap-2"
												style={{ 
													borderRadius: '8px', 
													fontSize: '13px',
													backgroundColor: isOutOfStock || isInsufficientBalance ? undefined : '#0d9488',
													borderColor: isOutOfStock || isInsufficientBalance ? undefined : '#0d9488',
													color: isOutOfStock || isInsufficientBalance ? undefined : '#fff'
												}}
			...
			>
				<Modal.Header closeButton={!submitting} className="border-bottom-0 pb-0">
					<Modal.Title className="fw-bold fs-5 d-flex align-items-center gap-2 text-dark">
						<Gift size={20} className="text-teal" style={{ color: '#0d9488' }} />
					...
					{selectedReward && (
						<>
							<p className="text-muted small mb-3">
								Apakah Anda yakin ingin menukarkan <strong>{selectedReward.title}</strong>? Poin lokal Anda untuk event ini akan dipotong langsung sebesar <strong>{selectedReward.points_cost} poin</strong>.
							</p>
							
							<div className="p-3 bg-light rounded-3 mb-3 border text-dark">
								<div className="d-flex justify-content-between small mb-1">
									<span>Saldo Poin Lokal Anda:</span>
...
export default LocalRewardCatalog;
