...
const AdminPaymentPage = () => {
    ...
    const filtered = transactions.filter((t) => {
        const matchQ = t.token.toLowerCase().includes(search.toLowerCase()) || 
    ...
    return (
        <div className="py-4 px-3 px-md-4" style={{ backgroundColor: 'var(--color-bg, #f8fafc)', minHeight: '100vh' }}>
            {/* Header Section */}
                ...
                <Card.Body className="p-0">
                    ...
                    <div className="bg-white p-3 p-md-4 border-bottom">
                        <Row className="g-3 align-items-center">
                            ...
                            <Col xs={12} md={7} lg={8} className="d-flex flex-wrap flex-md-nowrap justify-content-md-end gap-2 gap-md-3">
                                {/* Status Filter */}
                                <Form.Select
                                    value={statusFilter}
                                    onChange={(e) => handleStatusFilter(e.target.value)}
                                    className="shadow-none border text-muted"
                                    style={{ fontSize: '0.875rem', minWidth: '140px', flex: '1 1 auto', maxWidth: '200px' }}
                                >
                                ...
                                <div className="d-flex align-items-center gap-2">
                                    <span className="text-muted small d-none d-sm-inline">Tampilkan:</span>
                                    <Form.Select
                                        value={perPage}
                    ...
                    {loading ? (
                        <div className="d-flex flex-column justify-content-center align-items-center py-5">
                            ...
                            <span className="text-muted small fw-medium">Memuat data transaksi...</span>
                        </div>
                    ) : (
                        <div className="table-responsive">
                            <Table hover className="align-middle mb-0" style={{ fontSize: '0.875rem', minWidth: '900px' }}>
                                <thead className="bg-light text-muted">
                                    <tr>
                                        <th className="fw-semibold py-3 px-4 border-0">Token / Order ID</th>
                                        ...
                                        <th className="fw-semibold py-3 px-4 border-0 text-center">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {pageData.map((t) => (
                                        <tr key={t.id}>
                                            <td className="px-4">
                                                <div className="fw-bold text-primary font-monospace mb-1">{t.token}</div>
                                                <Badge bg="light" text="dark" className="border fw-normal font-monospace">
                                                    {t.order_id}
                                                </Badge>
                                            </td>
                                            <td>
                                                <div className="fw-bold text-dark">{t.customer_name}</div>
                                                <div className="text-muted small">{t.customer_email}</div>
                                            </td>
                                            <td className="text-truncate text-secondary" style={{ maxWidth: '180px' }}>
                                                {t.item_name}
                                            </td>
                                            ...
                                            <td>
                                                {t.status === 'success' && (
                                                    <Badge bg="success" className="bg-opacity-10 text-success border border-success border-opacity-25 px-2 py-1 rounded-pill fw-medium d-inline-flex align-items-center gap-1">
                                                        <CheckCircle2 size={12} /> Success
                                                    </Badge>
                                                )}
                                                {t.status === 'pending' && (
                                                    <Badge bg="warning" className="bg-opacity-10 text-warning border border-warning border-opacity-25 px-2 py-1 rounded-pill fw-medium d-inline-flex align-items-center gap-1">
                                                        <Clock size={12} /> Pending
                                                    </Badge>
                                                )}
                                                {t.status === 'failed' && (
                                                    <Badge bg="danger" className="bg-opacity-10 text-danger border border-danger border-opacity-25 px-2 py-1 rounded-pill fw-medium d-inline-flex align-items-center gap-1">
                                                        <XCircle size={12} /> Failed
                                                    </Badge>
                                                )}
                                                {t.status === 'expired' && (
                                                    <Badge bg="secondary" className="bg-opacity-10 text-secondary border border-secondary border-opacity-25 px-2 py-1 rounded-pill fw-medium d-inline-flex align-items-center gap-1">
                                        ...
                                        <tr>
                                            <td colSpan="7" className="text-center py-5">
                                                <div className="text-muted d-flex flex-column align-items-center gap-2">
                                                    ...
                                                    <span className="fw-medium">Belum ada data transaksi yang sesuai filter.</span>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
