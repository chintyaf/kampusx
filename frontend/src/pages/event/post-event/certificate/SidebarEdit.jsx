	...
	return (
		<div className="bg-white px-3 rounded-4 border cert-sidebar">
			...
			<div className="p-3 flex-grow-1 overflow-auto sidebar-scrollable">
				...
				<div className="mb-3">
					<label
						className="text-muted text-uppercase fw-bold small mb-2 d-flex justify-content-between"
						style={{ letterSpacing: '1px', fontSize: '0.7rem' }}
					>
						<span>{isQr ? 'Ukuran QR Code' : 'Ukuran Font'}</span>
						<span className="text-primary fw-bold">{selectedEl.fontSize || (isQr ? 80 : 24)}px</span>
					</label>
					{isQr ? (
						<Form.Range
							min={40}
						...
						<div className="d-flex flex-wrap gap-1">
							{FONT_SIZES.map((sz) => (
								<Button
									key={sz}
									variant={selectedEl.fontSize === sz ? 'dark' : 'outline-secondary'}
									size="sm"
