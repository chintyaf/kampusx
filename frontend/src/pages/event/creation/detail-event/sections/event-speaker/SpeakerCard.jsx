...
const SpeakerCard = ({ data, onEdit, onDelete }) => {
	...
	const renderSocialLinks = () => {
		if (!social_link || !Array.isArray(social_link) || social_link.length === 0) {
			return <span className="text-muted fst-italic small">Media sosial belum ada</span>;
		}
	...
	const getSessionText = () => {
		...
		return sessions.map((s) => s.name || s.title || s.label || `Sesi ${s.id || s}`).join(', ');
	};
				...
				<Row className="align-items-start g-3">
					...
					<Col xs="auto">
						{data.image_url ? (
							...
							<div
								...
								style={{ width: '70px', height: '70px' }}
							>
								<User size={35} className="text-secondary opacity-50" />
							</div>
						)}
					</Col>
					...
					<Col className="flex-grow-1">
						...
						<p className="text-muted mb-2 small">
							{role || <span className="fst-italic">Belum ada data</span>}
						</p>
