...
const ScheduleForm = ({ onClose, totalDays, sessionData, onSave, allSessions }) => {
				...
				if (reqSession) {
					...
					if (reqDay === currentDay && reqSession.startTime) {
						if (reqSession.startTime >= currentStartTime) {
							alert(
								`Validasi Gagal: Sesi prasyarat "${reqSession.title || 'Tanpa Judul'}" (Mulai: ${reqSession.startTime}) diagendakan lebih lambat atau bersamaan dengan sesi ini (Mulai: ${currentStartTime}).\nSolusi: Jadwalkan prasyarat lebih awal, sesuaikan waktu sesi ini, atau hapus prasyaratnya.`,
							);
					...
					if (reqDay > currentDay) {
						alert(
							`Validasi Gagal: Sesi prasyarat "${reqSession.title || 'Tanpa Judul'}" diagendakan di hari ke-${reqDay}, sedangkan sesi ini ada di hari ke-${currentDay}. Harap perbaiki urutan harinya.`,
						);
						return; // Batalkan proses simpan
					}
				}
	...
	return (
		<div className="p-1">
			<Form.Group controlId="formDaySelection" className="mb-4">
				<Form.Label className="small fw-bold text-muted">PILIH HARI</Form.Label>
				<Form.Select name="day" value={formData.day} onChange={handleChange} size="sm">
			...
			<Form.Group controlId="formTitle" className="mb-4">
				<Form.Label className="small fw-bold text-muted">JUDUL / AGENDA SESI</Form.Label>
				<Form.Control
					type="text"
					name="title"
					size="sm"
			...
			<Form.Group className="mb-4" controlId="formTimeRange">
				<Form.Label className="small fw-bold text-muted">RENTANG WAKTU</Form.Label>
				<InputGroup size="sm">
					<Form.Control
						className="no-clock"
						type="time"
			...
			<Form.Group className="mb-4">
				<Form.Label className="small fw-bold text-muted">PRASYARAT SESI</Form.Label>
				<Select
					isMulti
					name="prerequisites"
				...
				<Form.Text className="text-muted" style={{ fontSize: '0.75rem' }}>
					Kosongkan jika peserta bisa mengikuti sesi ini secara langsung.
				</Form.Text>
			</Form.Group>

			<div className="d-flex gap-2 justify-content-end mt-4 pt-3 border-top">
				<Button variant="light" size="sm" onClick={onClose} className="px-3 fw-medium">
					Batal
				</Button>
				<Button
					variant="primary"
