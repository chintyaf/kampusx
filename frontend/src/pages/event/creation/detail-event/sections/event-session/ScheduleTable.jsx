...
const ScheduleTable = ({ sessions = [], setSessions, totalDays }) => {
	...
	const handleSaveSession = (updatedSession) => {
		...
		updatedSessionsList.sort((a, b) => {
			...
			const timeA = a.startTime || (a.time ? a.time.split('–')[0] : '23:59');
			const timeB = b.startTime || (b.time ? b.time.split('–')[0] : '23:59');
		...
		return (
			<>
				<div className="text-center p-5 border rounded-3 bg-light text-muted">
					...
					<p className="small mb-0">Silakan tambah sesi jadwal terlebih dahulu.</p>
				</div>
	...
	return (
		<>
			<Table responsive className="form-table align-middle border rounded-4 mt-3">
				...
				<tbody>
					{sessions.map((s, idx) => {
						...
						const showDay = idx === 0 || sessions[idx - 1].day !== s.day;
						...
						return (
							<React.Fragment key={s.id}>
								<tr
									...
									className={activeRow === s.id ? 'table-active' : ''}
								>
									<td className="fw-bold">{showDay ? `Hari ${s.day}` : ''}</td>
									<td className="text-muted">Sesi {s.session}</td>
									<td>{displayTime}</td>
									...
									<td>
										...
										{s.prerequisites && s.prerequisites.length > 0 ? (
											...
											<Badge
												...
												className="fw-normal rounded-pill px-3"
											>
												<Lock size={12} className="me-1" /> Bersyarat
											</Badge>
										) : (
