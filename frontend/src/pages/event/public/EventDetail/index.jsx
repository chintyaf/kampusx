...
const EventDetail = () => {
    ...
    const handleToggleBookmark = async () => {
        ...
        try {
            ...
            setToastMsg(response.data.is_bookmarked ? 'Event disimpan ke Bookmark' : 'Event dihapus dari Bookmark');
            setShowToast(true);
        } catch (err) {
                ...
                <Container>
                    ...
                    <div className="d-flex justify-content-between align-items-start mb-3 gap-3">
                        <div className="flex-grow-1">
                            ...
                            <h1 className="fw-bold m-0" style={{ color: 'var(--color-text)', fontSize: '2.5rem', lineHeight: 1.2 }}>
                                {eventDetails.title}
                            </h1>
                    ...
                    <Col lg={8}>
                        ...
                        <div className="bg-white p-4 rounded-4 shadow-sm border mb-4">
                            ...
                            {eventDetails.sessions && eventDetails.sessions.length > 0 ? (() => {
                                const sessionsByDay = eventDetails.sessions.reduce((acc, session) => {
                                ...
                                return (
                                    <Accordion defaultActiveKey="day-1">
                                        {Object.keys(sessionsByDay).map((day) => (
                                            <Accordion.Item eventKey={`day-${day}`} key={day} className="mb-3 border rounded">
                                                <Accordion.Header>
                                                ...
                                                <Accordion.Body>
                                                    <ul className="list-unstyled m-0">
                                                        {sessionsByDay[day].sort((a, b) => (a.start_time || '').localeCompare(b.start_time || '')).map((session, index) => (
                                                            <li key={session.id || index} className="d-flex mb-4 align-items-start">
                                                                <div className="fw-bold me-3" style={{ minWidth: '90px', color: 'var(--color-primary)', fontSize: 'var(--font-sm)' }}>
                                                                    {session.start_time ? session.start_time.substring(0, 5) : '09:00'} - {session.end_time ? session.end_time.substring(0, 5) : '10:00'}
                                                                </div>
                                                                <div className="flex-grow-1">
                                                                    <div className="fw-semibold text-dark" style={{ fontSize: 'var(--font-sm)' }}>{session.title}</div>
                                                                    {session.description && <p className="text-muted small mb-2">{session.description}</p>}
                                                                    {session.speakers && session.speakers.length > 0 && (
                                                                        <div className="d-flex flex-wrap gap-2 mt-1">
                                                                            {session.speakers.map((speaker, idx) => (
                                                                                <span key={speaker.id || idx} className="badge bg-light text-dark border small py-1.5 px-2.5" style={{ borderRadius: '6px' }}>
                                ...
                                <p className="text-muted small m-0">Rundown sesi belum diumumkan oleh penyelenggara.</p>
                            )}
                        </div>
                    </Col>
