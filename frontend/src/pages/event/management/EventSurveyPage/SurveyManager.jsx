...
export default function SurveyManager() {
			...
			questions: [
				{
					id: '1',
					type: 'radio',
					label: 'Apakah Anda masih mengikuti acara ini?',
					required: true,
					options: ['Ya, saya hadir', 'Tidak'],
				},
			],
		},
		postEvent: {
			...
			questions: [
				{
					id: '1',
					type: 'rating',
					label: 'Berikan rating untuk acara ini',
					required: true,
				},
				{
					id: '2',
					type: 'textarea',
					label: 'Apa yang paling Anda sukai dari acara ini?',
					required: true,
				},
				{
					id: '3',
					type: 'textarea',
					label: 'Saran untuk perbaikan acara berikutnya',
					required: false,
				},
			],
		},
	...
	const addQuestion = (moduleId) => {
		...
		setModules({
			...
			[moduleId]: {
				...modules[moduleId],
				questions: [...modules[moduleId].questions, newQuestion],
			},
		});
	};
	...
	const deleteQuestion = (moduleId, questionId) => {
		setModules({
			...
			[moduleId]: {
				...modules[moduleId],
				questions: modules[moduleId].questions.filter((q) => q.id !== questionId),
			},
		});
	};
	...
	const updateQuestion = (moduleId, questionId, updates) => {
		setModules({
			...
			[moduleId]: {
				...modules[moduleId],
				questions: modules[moduleId].questions.map((q) =>
	...
	const toggleModuleRequired = (moduleId) => {
		setModules({
			...modules,
			[moduleId]: { ...modules[moduleId], isRequired: !modules[moduleId].isRequired },
		});
	};
	...
	const renderFormBuilder = (module) => (
		<div className="d-flex flex-column gap-4">
			<div
				className="rounded-4 p-4 text-white shadow position-relative overflow-hidden"
			...
			>
				...
				<div
					className="position-relative d-flex align-items-start justify-content-between mb-4 flex-wrap gap-3"
					style={{ zIndex: 1 }}
				>
					<div className="flex-grow-1">
						<Badge
							bg={module.enabled ? 'success' : 'secondary'}
							className="mb-3 d-inline-flex align-items-center gap-2 rounded-pill px-3 py-2"
						>
							<div
								className={`rounded-circle ${module.enabled ? 'bg-white' : 'bg-light'}`}
					...
					<div className="bg-white bg-opacity-25 rounded-4 p-3 d-flex align-items-center gap-3 backdrop-blur">
						...
						<Form.Check
							...
							onChange={() => toggleModuleEnabled(module.id)}
						...
						<div className="bg-white bg-opacity-25 rounded-4 p-3 border border-light border-opacity-25 backdrop-blur h-100">
							<div className="d-flex align-items-center justify-content-between mb-3">
								<div className="d-flex align-items-center gap-3">
									<div
										className={`p-2 rounded-3 ${module.isRequired ? 'bg-success' : 'bg-secondary bg-opacity-50'}`}
									>
										{module.isRequired ? (
									...
									<div>
										...
										>
											{module.isRequired ? 'Wajib diisi' : 'Opsional'}
										</div>
									</div>
								</div>
								<Form.Check
									...
									onChange={() => toggleModuleRequired(module.id)}
									style={{ transform: 'scale(1.2)' }}
								/>
							</div>
			...
			<div className="d-flex flex-column gap-3">
				{module.questions.map((question, index) => (
					<Card
						key={question.id}
						className="border-2 shadow-sm rounded-4"
					...
					>
						<Card.Body className="p-4 d-flex align-items-start gap-3">
							...
							<div className="flex-grow-1 d-flex flex-column gap-3">
								<div className="d-flex align-items-center gap-2 flex-wrap">
									...
									<div className="bg-light rounded-pill px-3 py-2 d-flex align-items-center gap-2 border">
										<Form.Check
											...
											onChange={(e) =>
												updateQuestion(module.id, question.id, {
								...
								{(question.type === 'select' ||
									...
									<div className="bg-light p-3 rounded-3 mt-2">
										<Badge
											bg="primary-subtle"
											className="text-primary rounded-pill mb-2"
										>
											Opsi Pilihan
										</Badge>
										<Form.Control
							...
							<Button
								...
								className="text-muted p-2 hover-bg-danger-subtle hover-text-danger rounded-3"
							>
								<Trash2 size={20} />
							</Button>
						</Card.Body>
	...
	const renderAnalytics = () => (
		<div className="d-flex flex-column gap-4">
			...
			>
				...
				>
					...
					<div>
						...
						<p className="mb-0" style={{ color: 'rgba(255,255,255,0.9)' }}>
							Visualisasi data survei yang masuk secara real-time
						</p>
					</div>
				</div>
			</div>
			...
			<Row className="g-4">
				<Col md={3}>
					<Card
						className="border-0 shadow-sm rounded-4 h-100 text-white p-3"
				...
				<Col md={3}>
					<Card
						className="border-0 shadow-sm rounded-4 h-100 text-white p-3"
						style={{ background: 'linear-gradient(to bottom right, #22c55e, #059669)' }}
					>
						<div className="d-flex justify-content-between mb-3">
							<TrendingUp size={24} /> Kepuasan
						</div>
	...
	return (
		...
		>
			<div className="mb-4 d-flex align-items-center gap-3">
				<div
					className="p-3 rounded-4 shadow-sm text-white"
				...
				<div>
					...
					<p className="mb-0 text-muted">
						Kelola formulir survei untuk berbagai tahap acara di KampusX
					</p>
				</div>
			</div>
			...
			{viewMode === 'builder' ? (
				<Tabs
					...
					className="mb-4 border-0 gap-2 custom-tabs"
				>
					<Tab
						eventKey="checkout"
						title={
							<span className="d-flex align-items-center gap-2 px-3 py-2">
								<span className="fs-5">📝</span> Checkout Question
							</span>
						}
					>
					...
					<Tab
						...
						title={
							<span className="d-flex align-items-center gap-2 px-3 py-2">
								<span className="fs-5">✅</span> Mid-Event Check
							</span>
						}
					>
					...
					<Tab
						...
						title={
							<span className="d-flex align-items-center gap-2 px-3 py-2">
								<span className="fs-5">⭐</span> Post-Event Feedback
							</span>
						}
					>
