 ...
 * eventStatus {String} Status dari event (contoh: 'Draft', 'Published')
 */
...
export default function MissingInformation({
	...
	// Cegah render komponen jika tidak ada isu sama sekali
	if (!issues || issues.length === 0) {
		return null;
	}

	// Cek apakah komponen perlu mendapat perhatian khusus
	const needsAttention = eventStatus === 'Draft' && issues.length > 0;
	...
	return (
		<>
          ...
          @keyframes pulse-amber {
            0% { box-shadow: 0 0 0 0 rgba(217, 119, 6, 0.4); border-color: #d97706; }
            70% { box-shadow: 0 0 0 6px rgba(217, 119, 6, 0); border-color: #fbbf24; }
        ...
        `}
			...
			<div className={`card h-100 ${needsAttention ? 'attention-border' : ''}`}>
