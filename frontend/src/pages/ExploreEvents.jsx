...
const ExploreEvents = () => {
	...
	const applyFilters = () => {
		...
		if (
			filters.locationType.includes('Online') &&
			!filters.locationType.includes('In-Person')
		) {
		...
		if (filters.price === 'Gratis')
			result = result.filter((ev) => !ev.price || ev.price === 0 || ev.price === '0');
		if (filters.price === 'Berbayar')
			result = result.filter((ev) => ev.price && ev.price !== 0 && ev.price !== '0');
						...
						<div style={{ marginBottom: 20 }}>
							<div
								style={{
									...
									flexWrap: 'wrap',
									gap: 8,
									marginBottom: activeCount > 0 ? 10 : 0,
								}}>
								<span
									style={{
										fontSize: 'var(--font-sm)',
										...
										<span key={t} style={chipStyle}>
											...
											<X
												...
												onClick={() =>
													handleChange(
														'locationType',
														applied.locationType.filter((x) => x !== t),
													)
												}
											/>
										</span>
									...
									{applied.price !== 'Semua' && (
										<span style={chipStyle}>
											...
											<X
												size={10}
												style={{ cursor: 'pointer', marginLeft: 2 }}
												onClick={() => handleChange('price', 'Semua')}
											/>
										</span>
									)}
									{applied.category && (
